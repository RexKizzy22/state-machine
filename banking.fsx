...
module DependencyInjection =
    ...
    let validateApplication (application: Application) = true

    let validateIds (ids: Ids) = true
...
module ValidStatesTest =
    type TwoIds = private Ids of Identification * Identification
...
module CheckingAccount =
    ...
    let application =
        makeCheckingAccountApplication firstName lastname homePhone emailAddress
...
module UnitOfMeasure =
    ...
    let openCheckingAccount (deposit: Deposit) =
        if deposit > 500M<money> then Some CheckingAccount else None
...
module Temperature =
    ...
    let badConvertDegCToF c = c * 1.8<degF / degC> + 32.0
